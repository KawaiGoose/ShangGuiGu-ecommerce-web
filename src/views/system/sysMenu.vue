...
const addShow = row => {
  ...
  if (row && row.id) {
    ...
    sysMenu.value.parentId = row.id // 设置选中行的 ID 作为新项的父ID
  } else {
    dialogTitle.value = '添加'
    sysMenu.value.parentId = 0 // 设置 parentId 为 0 表示顶级节点
  }
}
...
const saveOrUpdate = async () => {
  console.log('Saving or updating item with parentId:', sysMenu.value.parentId)
  if (!sysMenu.value.id) {
    await saveData() // 新增
  } else {
    await updateData() // 更新
